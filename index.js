    ...
    if(message.isMedia) {
      ...
      if(message.isGroupMsg) {
        ...
        client.downloadFile(message).then(async bufer => {
          ...
          const filename = `figurinhas/${b.replace('false', '')}.${mime.extension(message.mimetype).includes("jpeg") ? mime.extension(message.mimetype).replace('jpeg', 'png') : mime.extension(message.mimetype)}`;
      ...
      const sender = message.from;
        ...
        client.downloadFile(message).then(async buffer => {
          ...
          const fileName = `figurinhas/${c.replace('false', '')}.${mime.extension(message.mimetype)}`;
